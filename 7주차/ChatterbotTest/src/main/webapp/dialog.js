    ...
    $.fn.extend({ 
        ...
        appendDiv: function(setClass) {
            ...
            return this.each(function() {
                ...
                var chatter_box = $('<div></div>').appendTo(objDIV).addClass('chatter_box');
                var chatter_box1 = $('<div></div>').appendTo(objDIV).addClass('chatter_box1');
                var text_box = $('<input type=text id="txtName"></input>').appendTo(chatter_box1).addClass('text_box');
                var send_button = $('<input type="button" value="Send"></input>').appendTo(chatter_box1).addClass('send_button');
                
                send_button.click(function(){
                    $.ajax({
                        type:"POST", //HTTP 메서드, 기본값은 GET이다. POST와 GET 중 선택
                        url:"table.json",    //요청을 보낼 서버 URL
                        data: JSON.stringify({id:text_box.val()}), //JSON.stringify(text_box.val()),  //서버에 전송할 데이터 
                        dataType:"json",    // 응답결과의 데이터 형식 (xml,html,json,jsonp,script,text)
                        //timeout: "value", //요청에 대한 응답 제한시간(millisecond)
                        contentType: "application/json; charset=UTF-8", //서버 전송시의 content-type, application/x-wwwform-urlencoded 기본
                        success : function(data) {  //응답이 성공하는 경우 호출되는 콜백함수
                        	if(data.model==null) {
                        		//chatter_box.css("textAlign", "right");
                        		chatter_box.append("<p style=\"text-align:right\"><img src=\"com.jpg\" class=\"img_setting\" align=\"top\">컴퓨터 : DB에 등록되지 않은 명령어 입니다.</p>");
                        		chatter_box.scrollTop(chatter_box[0].scrollHeight);
                        	}else {
                        		chatter_box.append("<p style=\"text-align:right\"><img src=\"com.jpg\" class=\"img_setting\" align=\"top\">컴퓨터 : "+data.model+"</p>");
                        		chatter_box.scrollTop(chatter_box[0].scrollHeight);
                        	}
                        },
                    ...
                    //chatter_box.css("textAlign", "left");
                    chatter_box.append("<p style=\"text-align:left\"><img src=\"suzi.jpg\" class=\"img_setting\" align=\"top\">질문 : "+text_box.val()+"</p>");
                    chatter_box.scrollTop(chatter_box[0].scrollHeight);
                    text_box.val('');
                });
